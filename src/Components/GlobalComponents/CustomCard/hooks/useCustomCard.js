import {
  ...
  defaultAtrBtn,
  defaultAtrImage,
} from "../utilities/cutomCardSchema";
...
export const useCustomCard = (props) => {
  ...
  const { backgroundSize = "cover", height = "250px" } =
    customImage || defaultAtrImage;
  ...
  const classNameCardBody = `second-col col col-12 d-flex  ${
    gridCard[grid]["secondCol"]
  } ${ellipsis ? "justify-content-center" : ""} align-items-center p-2`;

  const classNameCardSubTitleText = `text-center ${
    ellipsis ? "col col-12 text-truncate" : ""
  } ${gridCard["text"][grid]}`;
