...
const personalSlides = (arraySlides) => {
  const filterArray = arraySlides.filter(
    (slide) => slide.order >= PERSONAL_ORDER
  );

  let hashOrders = {};
  const arrayWithoutOrderRepeat = filterArray.filter((item) =>
    hashOrders[item.order] ? false : (hashOrders[item.order] = true)
  );

  return arrayWithoutOrderRepeat;
};
